...
def get_role_list_resources(crversion,sessionname,token):
...
def role_list(outputFormat,sessionname):
    ...
    IsVersionSupported,CallType,ApiUri,Headers,Body = get_role_list_resources(CRVERSION,sessionname,TOKEN)
    ...
    if(not isAPICallOK):
        ...
        if (outputFormat == "DF"):
            #print(json_object)
            aDF = RolesTransformers.GetListAsCsv(json_object)
            print(aDF)
